...
int main(int argc, char **argv){
    ...
    ros::init(argc, argv, "fp_wrenchLeft_publisher");
    ros::NodeHandle n;
    ...
    //LOAD OF robot.mat
    iDynTree::MatrixDynSize forceplateWrenchLeftArmForces_i = matrixFromMatlabExtractor_byString("forceplate.mat","forceplate/data/plateforms/plateform2/forces");
    iDynTree::MatrixDynSize forceplateWrenchLeftArmMoments_i = matrixFromMatlabExtractor_byString("forceplate.mat","forceplate/data/plateforms/plateform2/moments");

    int j = 0;
    geometry_msgs::WrenchStamped wrenchMsgLeft;
    ros::param::param<std::string>("~frame_id", wrenchMsgLeft.header.frame_id, "forceplate2");

    while (ros::ok()){

        j++;
        if(j > (forceplateWrenchLeftArmForces_i.cols())) j = 0;

        //compose WrenchStamped Msg
        wrenchMsgLeft.header.stamp = ros::Time::now();
        wrenchMsgLeft.wrench.force.x = forceplateWrenchLeftArmForces_i.getVal(0,j);
        wrenchMsgLeft.wrench.force.y= forceplateWrenchLeftArmForces_i.getVal(1,j);
        wrenchMsgLeft.wrench.force.z = forceplateWrenchLeftArmForces_i.getVal(2,j);
        wrenchMsgLeft.wrench.torque.x = forceplateWrenchLeftArmMoments_i.getVal(0,j);
        wrenchMsgLeft.wrench.torque.y = forceplateWrenchLeftArmMoments_i.getVal(1,j);
        wrenchMsgLeft.wrench.torque.z = forceplateWrenchLeftArmMoments_i.getVal(2,j);

        wrench_pub.publish(wrenchMsgLeft);
