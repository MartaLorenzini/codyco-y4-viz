...
int main(int argc, char **argv){
    ...
    ros::init(argc, argv, "icub_wrenchLeft_publisher");
    ros::NodeHandle n;
    ...
    //LOAD OF robot.mat
    iDynTree::MatrixDynSize robotWrenchLeftArmForces_i = matrixFromMatlabExtractor_byString("robot.mat","robot/data/links/leftarm/forces");
    iDynTree::MatrixDynSize robotWrenchLeftArmMoments_i = matrixFromMatlabExtractor_byString("robot.mat","robot/data/links/leftarm/moments");

    int j = 0;
    geometry_msgs::WrenchStamped wrenchMsgLeft;
    ros::param::param<std::string>("~frame_id", wrenchMsgLeft.header.frame_id, "l_forearm");

    while (ros::ok()){

        j++;
        if(j > (robotWrenchLeftArmForces_i.cols())) j = 0;

        //compose WrenchStamped Msg
        wrenchMsgLeft.header.stamp = ros::Time::now();
        wrenchMsgLeft.wrench.force.x = robotWrenchLeftArmForces_i.getVal(0,j);
        wrenchMsgLeft.wrench.force.y= robotWrenchLeftArmForces_i.getVal(1,j);
        wrenchMsgLeft.wrench.force.z = robotWrenchLeftArmForces_i.getVal(2,j);
        wrenchMsgLeft.wrench.torque.x = robotWrenchLeftArmMoments_i.getVal(0,j);
        wrenchMsgLeft.wrench.torque.y = robotWrenchLeftArmMoments_i.getVal(1,j);
        wrenchMsgLeft.wrench.torque.z = robotWrenchLeftArmMoments_i.getVal(2,j);
